...
def _extract_voice_name(cli_args: list[str]) -> str:
    """Read the explicit --voice-name value from forwarded CLI arguments."""
    for index, item in enumerate(cli_args):
        if item == "--voice-name" and index + 1 < len(cli_args):
            return cli_args[index + 1]
        if item.startswith("--voice-name="):
            return item.split("=", 1)[1]
    return DEFAULT_VOICE_NAME


def run_intro_outro(
    root: Path, subject: str, videos: list[Path], cli_args: list[str]
) -> list[Path]:
    """Add narrated intro + outro cards to every final video automatically.

    Returns the list of deliverable videos (enhanced when possible, otherwise
    the original). A failure here never fails the whole task: the plain video
    stays the deliverable and the error is logged.
    """
    if os.environ.get("MPT_INTRO_OUTRO", "1").strip().lower() in {"0", "false", "no"}:
        log("intro/outro cards disabled via MPT_INTRO_OUTRO=0")
        return videos
    script = root / "scripts" / "add_intro_outro.py"
    ...
    enhanced: list[Path] = []
    for video in videos:
        if video.stem == "final-1":
            out = video.with_name("final-2.mp4")
        else:
        ...
        if result.returncode != 0:
            ...
            tail += (result.stderr or "").splitlines()[-5:]
            log(f"intro/outro failed for {video.name}; keeping original video")
            if tail:
                print("\n".join(tail), file=sys.stderr)
            enhanced.append(video)
        elif out.is_file() and out.stat().st_size > 0:
            enhanced.append(out.resolve())
        else:
            log(f"intro/outro produced no output for {video.name}; keeping original")
            enhanced.append(video)
    return enhanced
...
def generate_video(
    ...
    if not videos:
        ...
        raise SkillError(error)
    final_videos = run_intro_outro(root, subject, videos, cli_args)
    result_path = write_result_manifest(
        root,
        {
            ...
            "task_dir": str(task_dir.resolve()),
            "log_file": str(log_path.resolve()),
            "video_files": [str(video) for video in final_videos],
            "raw_video_files": [str(video) for video in videos],
        },
    )
    return final_videos, task_dir.resolve(), log_path.resolve(), result_path
