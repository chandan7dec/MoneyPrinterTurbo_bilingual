...
def create_fallback_subtitle(text: str, audio_duration: float, subtitle_file: str) -> bool:
    """Generate an evenly-timed SRT when TTS cue matching silently fails.

    Edge TTS cues can fail to align with scripts containing URLs, symbols, or
    unusual punctuation (cue/line count mismatch), and ``create_subtitle``
    deliberately returns without writing a file in that case. This fallback
    always yields readable captions so a missing subtitle file never silently
    drops subtitles from the final video. Returns True when the file is written.
    """
    text = _format_text(text)
    script_lines = utils.split_string_by_punctuations(text)
    if not script_lines or audio_duration <= 0:
        logger.warning(
            "fallback subtitle generation skipped: "
            f"lines={len(script_lines)}, audio_duration={audio_duration}"
        )
        return False
    formatter = _build_subtitle_formatter()
    duration = float(audio_duration)
    interval = duration / len(script_lines)
    sub_items = []
    for idx, line in enumerate(script_lines, start=1):
        start_time = int((idx - 1) * interval * 10000000)
        end_time = int(min(idx * interval, duration) * 10000000)
        sub_items.append(
            formatter(idx=idx, start_time=start_time, end_time=end_time, sub_text=line)
        )
