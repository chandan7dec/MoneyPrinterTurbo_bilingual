...
def generate_subtitle(task_id, params, video_script, sub_maker, audio_file):
    ...
    if subtitle_provider == "edge":
        ...
        if not os.path.exists(subtitle_path):
            # Edge 字幕偶尔会因为时间轴与文案无法匹配（例如脚本包含 URL 或
            # 特殊符号）而没有产出文件。这里先尝试本地兜底：把脚本断句按音频
            # 时长均匀分配生成 SRT，保证开启字幕时不会静默丢失字幕。不自动
            # 切换到 Whisper，避免首次失败时在用户不知情的情况下下载数 GB 模型。
            logger.warning(
                "edge subtitle generation did not produce a subtitle file; "
                "generating evenly-timed fallback subtitles"
            )
            audio_duration = voice.get_audio_duration(audio_file)
            created = voice.create_fallback_subtitle(
                text=video_script,
            ...
            if not created:
                logger.warning(
                    "fallback subtitle generation failed; skip subtitles "
