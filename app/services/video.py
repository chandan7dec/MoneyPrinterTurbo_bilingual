...
def _get_temp_audio_dir(output_dir: str) -> str:
    ...
    if sys.platform == "win32":
        # NOTE: this function is called *before* _prepare_task_temp_dir sets
        # TEMP/TMP, so on Windows we fall back to the system temp directory.
        # The actual temporary-file routing for both audio and video temps is
        # handled by _prepare_task_temp_dir + the TEMP/TMP env vars it sets,
        # which MoviePy/ffmpeg honour for every intermediate file.
        return tempfile.gettempdir()
    return output_dir


def _prepare_task_temp_dir(output_dir: str) -> str:
    """
    Prepare a per-task temp directory and set TEMP/TMP so MoviePy + ffmpeg
    write all temporary files (audio temps, video temps, work files) into the
    task directory instead of the shared system %TEMP%.

    On Windows this avoids WinError 32 collisions between concurrent renders
    that share the same output filename stem (for example the English + Hindi
    twin both writing ``final-1.mp4``).  Those tasks would otherwise create
    identically-named ``final-1TEMP_MPY_wvf_snd.*`` files in the shared
    %TEMP% and step on each other.  A per-task temp dir keeps every render's
    working files isolated by task folder.

    The directory is created under ``output_dir`` so it inherits the same drive
    and filesystem as the final render, which also avoids cross-filesystem temp
    moves on Windows.  The caller is responsible for cleaning the directory when
    the render finishes.
    """
    task_temp_dir = os.path.join(output_dir, "_moviepy_temp")
    os.makedirs(task_temp_dir, exist_ok=True)
    # MoviePy + ffmpeg pick up TEMP / TMP for any intermediate work files.
    # Point them at the per-task directory so concurrent renders never share
    # the same temporary name.  We overwrite (not just setdefault) because on
    # Windows the system %TEMP% is almost always already set, and we need every
    # intermediate file for *this* render to live in the per-task directory.
    # generate_video is short-lived, so we do not restore the previous values —
    # the process moves on to the next task afterwards.
    os.environ["TEMP"] = task_temp_dir
    os.environ["TMP"] = task_temp_dir
...
def generate_video(
    ...
    #
    # On Windows, MoviePy + ffmpeg spill intermediate work files into the
    # directory pointed at by TEMP / TMP.  When two renders run concurrently and
    # share the same output filename stem (for example the English + Hindi twin
    # both producing ``final-1.mp4`` in different task folders), they collide on
    # identically-named ``final-1TEMP_MPY_wvf_snd.*`` files in the shared
    # %TEMP% and fail with WinError 32.
   #
    # Fix: funnel every temporary file into a per-task temp directory that lives
    # next to the final render, so parallel tasks never share a temporary name.
    output_dir = os.path.dirname(output_file)
    task_temp_dir = _prepare_task_temp_dir(output_dir)

    # Windows can leave a locked ``TEMP_MPY_wvf_snd.*`` file behind when a
    # previous render was killed mid-write.  Stale locked files would make the
    # new render fail with WinError 32 on the first write attempt, so remove any
    # leftovers that are in our own per-task temp directory (we cannot unlink
    # files that belong to another process).
    #
    # Be careful *not* to delete the real final output file (for example
    # ``final-1.mp4``) — only the MoviePy/ffmpeg temporary work files whose
    # names contain the ``TEMP_MPY_wvf_snd`` marker or are recognisable audio/
    # video temp extensions inside the per-task temp directory.
    if sys.platform == "win32":
        _final_basename = os.path.basename(output_file)
        for _stale in os.listdir(task_temp_dir):
            if _stale == _final_basename:
                # Never touch the actual output file.
                continue
            if "TEMP_MPY_wvf_snd" in _stale:
                try:
                    os.remove(os.path.join(task_temp_dir, _stale))
                except OSError:
                    # File is locked by another process (or already gone) —
                    # leave it for the OS to clean up, the new render will use
                    # a fresh name if needed.
                    pass
    ...
    with ExitStack() as clip_stack:
        ...
        _write_videofile_with_codec_fallback(
            ...
            temp_audiofile_path=task_temp_dir,
